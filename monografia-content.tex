...
Um exemplo da representação interna de um módulo é demonstrada na
...
\begin{itemize}
\item A definição original por \citeonline{aMetricsSuiteForOO} - LCOM1;
\item A definição de \citeonline{maintenanceMetricsObjectOriented} - LCOM?;
\item A redefinição do LCOM de Li e Henry por \citeonline{measuringCouplingAndCohesion} - LCOM4; e
\item A redefinição da proposta original pelos próprios Chidamber e Kemerer - LCOM?.
\end{itemize}
...
acordo com \citeonline{measuringCouplingAndCohesion} faz mais sentido.
...
%A avaliação dos resultados dos grafos de colaboração e das métricas são
